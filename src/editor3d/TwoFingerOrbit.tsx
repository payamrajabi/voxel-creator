 ...
 *
 *  - One-finger drag   → orbit: horizontal swipe spins left/right (azimuth),
 *    vertical swipe tilts up/down (polar) — swipe down to look from overhead, up
 *    to drop back to a side-on view. A diagonal drag does both at once.
 *  - Two-finger twist  → also spins left/right (azimuth), Google-Earth style:
 *    rotate two fingers like a dial and the camera turns by the same angle.
 *
 * A one-finger *tap* still places/erases cubes and a one-finger *long-press*
 * still summons the glass box — orbit only kicks in once the finger has clearly
 ...
 * frame, so it never fights the position we set.
 */
...
// swipe down → tilt toward a top-down view; swipe right → spin like OrbitControls.
const ROT_SIGN = -1; // two-finger twist → azimuth
const AZI_SIGN = 1; // one-finger horizontal → azimuth
const TILT_SIGN = -1; // one-finger vertical → polar

// A full screen-height one-finger drag sweeps this much angle (azimuth & polar).
const DRAG_RADIANS_PER_SCREEN = Math.PI;
...
export default function TwoFingerOrbit({
  ...
  useEffect(() => {
    ...
    let dragging = false; // one-finger drag has passed the tap slop
    ...
    const onMove = (e: PointerEvent) => {
      ...
      // One finger → orbit: horizontal spins (azimuth), vertical tilts (polar),
      // once it's clearly a drag and not a tap.
      if (pts.size === 1) {
        if (singleStart === null) {
        ...
        if (!dragging) {
          ...
          dragging = true; // this frame's delta is small, so no jump
        }
        if (controlsBusy()) return;
        const height = el.clientHeight || 1;
        const dTheta = AZI_SIGN * ((p.x - prevX) / height) * DRAG_RADIANS_PER_SCREEN;
        const dPhi = TILT_SIGN * ((p.y - prevY) / height) * DRAG_RADIANS_PER_SCREEN;
        orbitAroundTarget(camera, targetOf(), dTheta);
